...
#                3. LOAD & PREPARE DATA 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
file_path <- here("1_PEB_main","Establish_LIS_priors_data.csv")  # Adjust the path if needed
...
plot(refineR_obj, Scale = "transformed") # The Box-Cox refineR model used for priors
plot(refineR_obj, Scale = "original") # The refineR estimated reference intervalon original scale 
...
p_main <- ggplot(plot_data, aes(x = bc_Baseline, y = bc_New)) +
  ...
  geom_line(aes(y = predicted_bc), linewidth = 1, color = "#36454f") +
  labs(
...
p_resid <- ggplot(plot_data, aes(x = residual_bc)) +
  ...
  geom_vline(xintercept = 0, linetype = "dashed", color = "#36454f", linewidth = 0.5) +
...
caption_text <- sprintf("**B1 = %.3f** at the Box-Cox scale **Lambda = %.3f**", B1_LIS, lambda)
...
summary_table <- data.frame(
  LIS_priors = c("Box-Cox lambda", 
                ...
                "Intraclass Correlation (Box-Cox scale)"),
  Variable_name = c("lambda", "sigma_pop", "mu_pop", "B1"),
